<template>
    <div class="app-container">
      ...
      >
        ...
        <el-form-item label="课程难度" prop="difficulty">
          <el-select v-model="queryParams.difficulty" placeholder="请选择课程难度" clearable>
            <el-option
      ...
      >
        ...
        <el-table-column label="课程名称" width="130" align="center" prop="title" />
        <el-table-column label="授课老师" align="center" prop="teacherId" width="80" :formatter="formatUserName" />
        <el-table-column label="课程描述" align="center" show-overflow-tooltip prop="description" />
        <el-table-column label="课程难度" width="80" align="center" prop="difficulty">
          <template #default="scope">
            <dict-tag :options="difficult_value" :value="scope.row.difficulty" />
          </template>
        </el-table-column>
        <el-table-column label="课程封面" align="center" prop="coverImage" width="100">
          <template #default="scope">
            <el-image :src="scope.row.coverImage" @click.stop :width="50" :height="50" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-button  type="text" icon="Edit" @click.stop="handleUpdate(scope.row)">修改</el-button>
            <el-button  type="text" icon="Edit" @click.stop="goToQuestion(scope.row)">题目设置</el-button>
            <el-button  type="text" icon="Delete" @click.stop="handleDelete(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-dialog :title="title" v-model="open" width="500px" append-to-body>
        <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
          ...
          <el-form-item label="授课老师" prop="teacherId">
            <el-select v-model="form.teacherId" placeholder="请选择授课老师" clearable>
              <el-option
  ...
  const rules = ref({
    ...
    difficulty: [{ required: true, message: '课程难度不能为空', trigger: 'change' }],
    coverImage: [{ required: true, message: '课程封面不能为空', trigger: 'blur' }],
    teacherId: [{ required: true, message: '授课老师不能为空', trigger: 'change' }],
  })
  ...
  const getList = async () => {
    try {
      loading.value = true
      const [coursesRes, userRes] = await Promise.all([listCourses(queryParams.value), listUser()])
  ...
  const handleDelete = async (row?: Course) => {
    ...
    try {
      await ElMessageBox.confirm(`是否确认删除课程编号为"${deleteIds}"的数据项？`, '警告', {
        type: 'warning',
      })
  ...
  const handleExport = () => {
    const url = `/system/courses/export?${new URLSearchParams(queryParams.value as any).toString()}`
    fetch(url)
